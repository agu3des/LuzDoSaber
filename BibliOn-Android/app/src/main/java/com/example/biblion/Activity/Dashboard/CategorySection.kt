...
import kotlin.jvm.java // (Essa linha parece desnecessária aqui)

// Função que exibe uma seção de categorias
@Composable
fun CategorySection(categories: SnapshotStateList<CategoryModel>, showCategoryLoading: Boolean) { // Define a função que mostra a seção de categorias
    Text(
        text = "Escolha a categoria", // Texto do título
        fontSize = 18.sp, // Tamanho da fonte
        fontWeight = FontWeight.Bold, // Texto em negrito
        modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp) // Espaçamento ao redor do texto
    )
    ...
    if (showCategoryLoading) { // Se estiver carregando categorias
        ...
        val rows = categories.chunked(3) // Divide as categorias em grupos de 3 para exibição em linhas
                    ...
                    row.forEachIndexed { index, categoryModel -> // Para cada categoria na linha
                        CategoryItem(
                            ...
                            onItemClick = { // Ação ao clicar na categoria
                                val intent = Intent(context, ItemsListActivity::class.java).apply { // Cria a intent para abrir a lista de itens
                                    putExtra("id", categoryModel.CategoryId.toString()) // Passa o ID da categoria
                                    putExtra("title", categoryModel.CategoryName) // Passa o nome da categoria
                                }
...
@Composable
fun CategoryItem(category: CategoryModel, modifier: Modifier = Modifier, onItemClick: () -> Unit) { // Parâmetros do item
    Column(
        modifier = modifier
