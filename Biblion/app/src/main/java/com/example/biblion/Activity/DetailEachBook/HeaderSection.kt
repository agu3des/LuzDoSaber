...
import com.example.biblion.Domain.BookModel // importa o modelo de dados do alimento
import com.example.biblion.R // importa recursos do projeto (como imagens e cores)

@Composable
fun HeaderSection(
    item: BookModel, // dado do alimento a ser exibido
    ...
    ConstraintLayout(
        ...
        Image(
            painter = rememberAsyncImagePainter(model = item.ImagePath), // carrega a imagem do alimento de forma assíncrona
        ...
        Text(
            text=item.Title, // título do alimento
        ...
        }) // linha com detalhes do alimento
        NumberRow(
            item=item, // dados do alimento
...
private fun FavoriteButton(modifier: Modifier=Modifier){
    Image(
        painter = painterResource(R.drawable.fav_icon), // ícone de favorito
        contentDescription = null, // descrição
        modifier = modifier
            .padding(end = 16.dp,top=48.dp) // espaçamento: 16dp na direita e 48dp no topo
    )
}