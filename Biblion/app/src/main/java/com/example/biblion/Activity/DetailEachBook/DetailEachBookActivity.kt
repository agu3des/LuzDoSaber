...
class DetailEachBookActivity : AppCompatActivity() { // Define a atividade que mostra detalhes de um alimento
    private lateinit var item: BookModel // Declara variável que armazena o alimento, será inicializada depois
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // Método chamado ao criar a atividade
        ...
        item = intent.getSerializableExtra("object") as BookModel // Recebe o objeto do alimento enviado pela intent
...
private fun DetailScreen(
    item: BookModel, // Alimento a ser exibido
    ...
    ConstraintLayout { // Layout que organiza elementos com restrições
        ...
        Column(modifier = Modifier // Container vertical para conteúdo
            ...
            HeaderSection( // Seção do cabeçalho
                ...
                onBackClick = onBackClick, // Função ao clicar para voltar
                onIncrement = { // Ao aumentar quantidade
                    numberInCart++ // Incrementa a quantidade
                    item.numberInCart=numberInCart // Atualiza o modelo
                },
                onDecrement = { // Ao diminuir quantidade
                    if(numberInCart>1){ // Só diminui se for maior que 1
                        numberInCart-- // Diminui a quantidade
                        item.numberInCart=numberInCart // Atualiza o modelo
                    }
                }
            )
            DescriptionSection(item.Description) // Seção de descrição do alimento
        }
        FooterSection(
