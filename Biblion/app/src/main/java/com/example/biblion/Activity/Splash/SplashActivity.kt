...
class SplashActivity : AppCompatActivity() { // Classe que representa a tela de splash
    override fun onCreate(savedInstanceState: Bundle?) { // Função chamada na criação da activity
        super.onCreate(savedInstanceState) // Chama a implementação pai9

        setContent { // Define o conteúdo usando Compose
            SplashScreen (onGetStartedClick = { // Chama a função SplashScreen, passando o evento de clique
                startActivity(Intent(this, MainActivity::class.java)) // Inicia a tela principal ao clicar
            })
        }
    }
}
